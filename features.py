...
def calculate_recent_form(team_id, days=21, max_games=10):
    ...
    games = get_recent_games(team_id, start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d"))
    ...
    for game in completed:
        ...
        if home["team"]["id"] == team_id:
            scored = int(home.get("score", 0))
            allowed = int(away.get("score", 0))
        else:
            scored = int(away.get("score", 0))
            allowed = int(home.get("score", 0))
    ...
    return {
        ...
        "recent_games": wins + losses,
        "win_pct": round(wins / games_played, 3),
        "runs_per_game": round(runs_scored / games_played, 2),
        "runs_allowed": round(runs_allowed / games_played, 2),
        "run_diff": round((runs_scored - runs_allowed) / games_played, 2),
    }
...
def pitcher_rating(p):
    era = safe_float(p.get("era"), 4.50)
    whip = safe_float(p.get("whip"), 1.35)
    k9 = safe_float(p.get("k9"), 8.0)
    bb9 = safe_float(p.get("bb9"), 3.2)
    last5_era = safe_float(p.get("last5_era"), era)
    last5_whip = safe_float(p.get("last5_whip"), whip)
...
def pitcher_features(player_id):
    if not player_id:
        return default_pitcher_features()

    stats = get_pitcher_stats(player_id, CURRENT_SEASON)

    try:
        split = stats["stats"][0]["splits"][0]["stat"]
        era = safe_float(split.get("era"), 4.50)
        whip = safe_float(split.get("whip"), 1.35)
        k9 = safe_float(split.get("strikeoutsPer9Inn"), 8.0)
        bb9 = safe_float(split.get("walksPer9Inn"), 3.2)
    except Exception:
        era, whip, k9, bb9 = 4.50, 1.35, 8.0, 3.2
    ...
    features["rating"] = pitcher_rating(features)
    return features

def pitcher_last5(player_id, fallback_era, fallback_whip):
    try:
        logs = get_pitcher_game_log(player_id, CURRENT_SEASON)
        splits = logs["stats"][0].get("splits", [])[:5]
        innings = earned_runs = walks_hits = 0.0

        for game in splits:
            stat = game.get("stat", {})
            ip = parse_ip(stat.get("inningsPitched", 0))
            earned_runs += safe_float(stat.get("earnedRuns"), 0)
            walks_hits += safe_float(stat.get("hits"), 0) + safe_float(stat.get("baseOnBalls"), 0)
            innings += ip

        if innings > 0:
            return (earned_runs * 9) / innings, walks_hits / innings
    except Exception:
        pass
...
def offense_rating_from_ops(ops):
    rating = 50 + (ops - 0.700) * 120
    return round(max(20, min(100, rating)), 1)

def offense_features(team_id, opposing_pitcher_hand):
    try:
        stats = get_team_hitting_splits(team_id, CURRENT_SEASON, opposing_pitcher_hand)
        split = stats["stats"][0]["splits"][0]["stat"]
        ...
        ops, avg, obp, slg = 0.700, 0.240, 0.310, 0.390
...
def season_team_stats(team_id):
    try:
        stats = get_team_stats(team_id, CURRENT_SEASON)
        split = stats["stats"][0]["splits"][0]["stat"]
        return {
            "runs": safe_float(split.get("runs"), 0),
            "hits": safe_float(split.get("hits"), 0),
            "ops": safe_float(split.get("ops"), 0.700),
            "avg": safe_float(split.get("avg"), 0.240),
            "obp": safe_float(split.get("obp"), 0.310),
            "slg": safe_float(split.get("slg"), 0.390),
...
def bullpen_features(team_id):
    try:
        stats = get_team_pitching_stats(team_id, CURRENT_SEASON)
        split = stats["stats"][0]["splits"][0]["stat"]
        era = safe_float(split.get("era"), 4.20)
        whip = safe_float(split.get("whip"), 1.35)
    except Exception:
        era, whip = 4.20, 1.35
    ...
    return {
        ...
        "bullpen_whip": whip,
        "bullpen_usage": usage,
        "rating": bullpen_rating(era, whip, usage),
    }
...
def build_features(schedule):
    ...
    for game in schedule:
        ...
        features.append({
            "home": home,
            "away": away,
            "home_form": calculate_recent_form(home_id),
            "away_form": calculate_recent_form(away_id),
            "home_pitcher": pitcher_features(game.get("home_pitcher_id")),
            "away_pitcher": pitcher_features(game.get("away_pitcher_id")),
            ...
            "home_team": season_team_stats(home_id),
            "away_team": season_team_stats(away_id),
            "home_bullpen": bullpen_features(home_id),
            "away_bullpen": bullpen_features(away_id),
